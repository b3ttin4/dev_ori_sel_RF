...
def parameter_sweep_layer4(Version,config_dict,**kwargs):
	...
	c_orth,s_orth = misc.get_projection_operators(config_dict,arbor_on,arbor_off)
	print("TEST",c_orth.shape)

	if config_dict["multiplicative_normalisation"]=="xalpha":
		...
		for i in range(num_pops):
			...
			# dot_product = np.dot(c_coeff,np.dot(c_orth,Wpop[arbpop>0]))
	...
	print("init_weights",init_weights[0,:5],init_weights[1,:5])
	...
	if config_dict["Inp_params"]["simulate_activity"]:
		if not kwargs["not_saving_temp"]:
			...
			print("yt",yt[-1,:])
			# print("l4t",l4t[-1,:],np.nanmean(l4t[-1,:]))

			Wfinal = y[:config_dict["num_lgn_paths"]*s].numpy().reshape(config_dict["num_lgn_paths"],\
																N4**2*Nvert,Nlgn**2)
			# print("CHECK NORM INIT",np.sum(Wlgn_to_4,axis=(0,2)),np.sum(Wlgn_to_4,axis=1))
			# print("CHECK NORM",np.sum(Wfinal,axis=(0,2)),np.sum(Wfinal,axis=1))
		...
		params_dict["W4to4"] =\
		 tf.convert_to_tensor(W4to4[:N4**2*Nvert,:N4**2*Nvert], name='W4to4', dtype=tf.float32)
		y0 = tf.convert_to_tensor(Wlgn_to_4.reshape(2*s), dtype=tf.float32)
		yt = integrator_tf.odeint(dynamics.dynamics_Wonly, y0, t, dt, params_dict,mode="static")
	...
	if config_dict["Inp_params"]["simulate_activity"]:
		...
		data_dict = {"W" : y[:config_dict["num_lgn_paths"]*s]}
